...
- 单元+集成测试：`src/**/*.test.js`（当前 36 个测试均通过，覆盖 orchestrator/memory/modifier/llm（含熔断+指标）、文件系统集成、patch merge（含 `patch-ast` 语法守卫）、真实模块编排修复链路、以及主进程 IPC 处理器）。
...
- CI：`.github/workflows/ci.yml` 已按分层执行：`push/PR` 运行 `npm test + e2e smoke`；`schedule/workflow_dispatch` 运行 `e2e smoke/full` matrix，并输出聚合报告（`GITHUB_STEP_SUMMARY`）。当 nightly full 失败时，会自动提取失败用例进入 quarantine rerun（隔离重跑），并把 flaky 统计写入历史缓存做趋势追踪（30 天窗口 + 按天衰减 score）；同时启用 flake gate（warn/block 阈值）联动告警与阻断，且阈值支持按分支默认值、仓库变量覆盖以及静默窗口（维护窗口）warn-only 模式，并输出静默窗口命中率、分支维度面板与最近 10 次趋势时间线。新增 Prometheus 指标导出（`reports/flake-metrics.prom`）与 Pushgateway 可选上报、7/30 SLO burn-rate 基线计算，并支持按严重度路由到 warn/block 告警与 primary/secondary on-call 升级。
...
- 外部告警 MVP：已支持配置 Webhook URL/Secret，并在健康告警触发时通过 `alert:webhook` 推送（支持 `v2/v1` 版本协商、tenant/key-id 头、多租户密钥轮换、去重时间窗、重试退避与失败落盘队列）。新增双向 ACK 与死信回放接口（`alert:webhook:ack`、`alert:deadletter:list`、`alert:deadletter:replay`）；协议文档补充接收端安全基线（IP 白名单 + nonce/timestamp 重放防护）。

## 4. 下一步建议

1. 为 ACK/死信链路补充签名验真与幂等约束（防止补偿接口误触发）。
2. 为 SLO 升级策略补充“抑制窗口 + 值班排班映射 + 自动降级恢复”闭环。
